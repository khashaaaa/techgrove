...
export const Register = () => {
  ...
  const registerUser = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/customer', options);
  ...
  return (
    <AuthLayout>
      ...
      <Link to="/login" className="flex items-center justify-end text-xs text-white bg-blue-400 rounded-xl px-2"><IconArrowRight /> эсвэл нэвтрэх</Link>
      <div className="grid grid-rows-4 gap-4 mt-8 w-80 text-sm">
        ...
        <button onClick={registerUser} className="bg-emerald-600 text-white rounded-xl px-4 py-2 focus:ring-4 focus:ring-emerald-200 hover:bg-emerald-500 delay-100">Болсон</button>
        <SocialAuth />
      </div>
    </AuthLayout>
  );
