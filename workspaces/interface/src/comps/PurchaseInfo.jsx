    ...
    return (
        <div className="mt-8 w-full flex justify-center items-center">
            ...
            <div className="flex justify-between rounded-2xl border-dashed border-2 border-stone-400 p-4 relative">
                <IconX onClick={() => close({})} className="absolute top-1 right-1 cursor-pointer" />
                <div>
                    <img style={{ height: "200px" }} className="w-full rounded-2xl" src={`/${productImage}`} alt="" />
                </div>
                <div className="ml-8 flex flex-col justify-between">
                    ...
                    <div className="mt-8 flex justify-between items-center">
                        <p className="font-bold">Дүн: <NumericFormat value={price} suffix={'₮'} thousandSeparator="," /></p>
                        <button onClick={() => InsertCart()} className="bg-green-800 hover:bg-green-600 font-bold text-white text-sm rounded px-3 py-1 ml-4">Сагслах</button>
                    </div>
                </div>
            </div>
        </div>
